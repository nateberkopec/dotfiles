function check_secrets
    ...
    set remediation "Real secrets must live in fnox/1Password. For false positives, add the printed fingerprint to .gitleaksignore or annotate the line with a 'gitleaks:allow' comment. To baseline current findings, run: gitleaks dir --redact=75 --report-path \"$target_dir/.gitleaks-baseline.json\" \"$target_dir\"; inspect its structure without displaying findings and confirm it contains no recovered values before committing"
