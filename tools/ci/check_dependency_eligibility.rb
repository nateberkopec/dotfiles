...
context = JSON.parse(File.read(ARGV.fetch(0), encoding: "UTF-8").tap { |text| abort "Invalid UTF-8 JSON" unless text.valid_encoding? })
data = JSON.parse(File.read(ARGV.fetch(1), encoding: "UTF-8").tap { |text| abort "Invalid UTF-8 JSON" unless text.valid_encoding? })
