...
allowed = {
  ...
  "update_pull_request" => %w[type pull_request_number pr pr_number body operation repo],
  "add_comment" => %w[type item_number pr pr_number body repo temporary_id], "noop" => %w[type message],
...
items.each do |item|
  ...
  if %w[update_pull_request push_to_pull_request_branch add_comment].include?(type)
    targets = item.slice("pull_request_number", "item_number", "pr", "pr_number").values
    abort "Unexpected target" unless context["number"] && !targets.empty? && targets.all? { |target| target.to_s == context["number"].to_s }
  end
