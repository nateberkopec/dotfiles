...
Use canonical names and individual gems. Account for every eligible/latest version newer than current, plus selected intermediate versions. Actions: `update`/`defer`; outcomes: `ready`/`blocked`/`deferred`/`researched`. Sources must match that release's collected evidence. Unavailable evidence uses `security: "unknown"`; null sources are allowed only for deferral when the collector also has none. Affirmative security claims need `quote`: at least 20 exact characters from collected notes. Generate repetitive JSON with a script.

**Publish.** Write `pr-body.md` in the shared directory. With `bundle exec ruby`, run saved `checks/check_dependency_report.rb CANDIDATES_JSON REPORT_MD BASE_SHA NOTES_JSON` and `checks/check_dependency_update.rb BASE_SHA`, plus relevant tests/lints. Commit before create/push and publish that exact checkout/body. Revisions require explicit context PR numbers and a replacement body; do not edit afterward. Failed, missing, or skipped post-validation blocks publication of the serialized safe output.
