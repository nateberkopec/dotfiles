...
class ConfigureDockStepTest < StepTestCase
  ...
  def setup
    super
    write_config("dock", dock_config)
  end

  def test_run_applies_all_dock_settings
    step.run

    assert_defaults_written(dock_entries)
  end
  ...
  def test_incomplete_when_autohide_differs
    stub_complete_settings(overrides: {"autohide" => 0})
    assert_incomplete
  end

  def test_incomplete_when_orientation_differs
    stub_complete_settings(overrides: {"orientation" => "bottom"})
    assert_incomplete
  end

  def test_incomplete_when_autohide_delay_differs
    stub_complete_settings(overrides: {"autohide-delay" => 1})
    assert_incomplete
  end

  def test_incomplete_when_autohide_time_modifier_differs
    stub_complete_settings(overrides: {"autohide-time-modifier" => 1.0})
    assert_incomplete
  end
  ...
  def test_incomplete_when_command_fails
    stub_complete_settings(status_overrides: {["com.apple.dock", "autohide"] => 1})
    assert_incomplete
  end
  ...
  def dock_entries
    @dock_entries ||= flatten_defaults_config(dock_settings)
  end
  ...
  def stub_complete_settings(overrides: {}, status_overrides: {}, persistent_apps: "(\n)", persistent_others: nil)
    stub_defaults(dock_entries, overrides: {"com.apple.dock" => overrides}, status_overrides: status_overrides)
    stub_defaults_reads("com.apple.dock", {
